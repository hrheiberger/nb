  ...
  <div class="app-wrapper">
    ...
    <div v-else>
      ...
      <div v-if="currentTab === 'classes'" class="tab">
        <div class="nb-side">
          <div v-for="course in courses" @click="selectCourse(course)">
            {{ course.class_name }}
          </div>
        </div>
        <div class="nb-main">
            ...
            <fieldset>
              ...
              <table>
                <tr
                  ...
                  v-bind:class="{ true: config.value === 'true' }"
                >
                  <td>{{ config.name }}</td>
                  <td>
          ...
          <template v-if="selectedCourse.class_name">
            ...
            <fieldset>
              ...
              <ul>
                <li>
                  ID: <code>{{ selectedCourse.id }}</code>
                </li>
                <li>
                  createdAt: <code>{{ selectedCourse.createdAt }}</code>
                </li>
                <li>
                  Creator:
                  <code
                    >{{ selectedCourse.Creator.name.first }}
                    {{ selectedCourse.Creator.name.last }}</code
                  >
                </li>
                <li>
                  # of Instructors:
                  <code>{{ selectedCourse.Instructors.length }}</code>
                </li>
                <li>
                  # of Sections:
                  <code>{{ selectedCourse.Sections.length }}</code>
                </li>
                <li>
                  # of Sources: <code>{{ selectedCourse.Sources.length }}</code>
                </li>
                <li>
                  # of Students:
                  <code>{{ selectedCourseStudents.length }}</code>
                </li>
              </ul>
            </fieldset>
            <fieldset>
              ...
              <table class="class-configs">
                <tr v-for="(value, name) in selectedCourseConfigs">
                  ...
                  <td v-if="name.startsWith('CONFIG_')">
                    <input
                      v-bind:class="{ 'selected-config': value !== 'Global' }"
                      :value="value"
                      @input="setConfig(name, $event.target.value)"
                    />
                    <button
                      v-bind:class="{ 'selected-config': value == 'Global' }"
                      @click="setConfig(name, 'Global')"
                    >
                      Global
                    </button>
                  </td>
                  <td v-else>
                    <button
                      v-bind:class="{ 'selected-config': value == 'true' }"
                      @click="setConfig(name, 'true')"
                    >
                      true
                    </button>
                    <button
                      v-bind:class="{ 'selected-config': value == 'false' }"
                      @click="setConfig(name, 'false')"
                    >
                      false
                    </button>
                    <button
        ...
        <fieldset>
          ...
          <table>
            <tr v-for="consent in consents">
              <details>
                <summary>
                  {{ consent.name }} ({{ consent.id }}) ✔️:{{
                    consent.Consentees.length
                  }}
                  ❌:{{ consent.Dissenters.length }}
                </summary>
                ✔️:
                <ul>
  ...
  watch: {
    selectedCourse: async function (newCourse, oldCourse) {
      ...
      const configsRes = await axios.get(
        `/api/admin/course/${this.selectedCourse.id}/configs`,
        headers
      );
      const courseRes = await axios.get(
        `/api/admin/course/${this.selectedCourse.id}/info`,
        headers
      );
      const selectedCourseStudents =
        courseRes.data.GlobalSection.MemberStudents;

      if (configsRes.data) {
        let course = JSON.parse(configsRes.data.value);
        course.configs.forEach(
          (c) => (this.selectedCourseConfigs[c.name] = c.value)
        );
      }
  ...
  methods: {
    ...
    updateConfig: async function () {
      if (window.confirm("Are sure?")) {
        ...
        const token = localStorage.getItem("nb.user");
        const headers = { headers: { Authorization: "Bearer " + token } };
        const updateConfigRes = await axios.post(
          `/api/admin/course/${this.selectedCourse.id}/configs`,
          course,
          headers
        );
    ...
    assignSource: async function (source) {
      ...
      const headers = { headers: { Authorization: "Bearer " + token } };
      const updateConfigRes = await axios.post(
        `/api/admin/course/${this.selectedCourse.id}/source/${source.id}/assignment`,
        {},
        headers
      );
      const assignmentsRes = await axios.get(
        `/api/admin/course/${this.selectedCourse.id}/assignments`,
        headers
      );
      this.assignedSources = assignmentsRes.data.map((s) => s.source_id);
    },
    setExpSpotlightGroups: function (
      selectedCourseStudents,
      control,
      treatment
    ) {
      let treatmentStudents = JSON.parse(
        JSON.stringify(selectedCourseStudents)
      );
      let controlStudents = [];
